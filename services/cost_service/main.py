...
async def lifespan(app: FastAPI):
    ...
    await init_redis()
    await init_kafka()
    print("Cost service started successfully")
    ...
    await close_redis()
    await close_db()
    print("Cost service shutdown completed")


# 创建 FastAPI 应用
app = FastAPI(
    title="成本中心",
    description="ERP 系统 - 成本中心微服务（成本核算、标准成本、分摊规则）",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/cost/docs",
    openapi_url="/cost/openapi.json",
)
...
async def health_check():
    """健康检查"""
    return {"status": "healthy", "service": "cost_service"}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(
        "services.cost_service.main:app",
        host="0.0.0.0",
        port=8006,
        reload=True
    )

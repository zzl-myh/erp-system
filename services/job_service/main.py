...
async def lifespan(app: FastAPI):
    ...
    await close_redis()
    await close_db()
    print("Job service shutdown completed")


# 创建 FastAPI 应用
app = FastAPI(
    title="报工中心",
    description="ERP 系统 - 报工中心微服务（报工、报损、工时管理）",
    version="1.0.0",
    lifespan=lifespan,
    docs_url="/job/docs",
    openapi_url="/job/openapi.json",
)
...
# 健康检查端点
@app.get("/job/health", tags=["健康检查"])
async def health_check():
    """健康检查"""
    return {"status": "healthy", "service": "job_service"}


@app.get("/job/ready", tags=["健康检查"])
async def readiness_check():
    """就绪检查"""
    # TODO: 检查数据库、Redis、Kafka 连接
    return {"status": "ready", "service": "job_service"}
